/* $Id: mcl_periodic_proc.cpp,v 1.2 2004/01/30 16:27:43 roca Exp $ */
/*
...
mcl_periodic_proc::scan_all_sessions (void)
{
	...
	int		mcl_sysinfo_count = MCL_SYSINFO_PERIOD / MCL_PERIODIC_TIMER_PERIOD;
#endif
...
mcl_periodic_proc::do_periodic_proc (mcl_cb	*mclcb)
{
	...
	int		eltc;		/* elasped time_count since last call */
	float		true_tx_tick_nb;/* tx tick elapsed as a float */
	int		tx_tick_nb;	/* integral part of true_tx_tick_nb */
	...
	if (mclcb->is_a_sender()) {
		...
		printf("eltc=%d, elit=%d, true_tx_tick_nb=%f, rate=%f pkt/tick, elit(sec)=%f, remaining_tx_tick_nb=%f, tx_tick_nb=%d\n",
		eltc, elit, true_tx_tick_nb, mclcb->tx.get_pkt_per_tick_tx_rate(), mcl_it2sec(elit),
		this->remaining_tx_tick_nb, tx_tick_nb);
#endif // 0
	 ...
	 */
	if (mclcb->sender && mclcb->delivery_mode == DEL_MODE_STREAMING
	...
	//
	//	mclcb->tx_mem_cleanup_rem_it = mcl_itime_sub(mclcb->tx_mem_cleanup_rem_it, elit);
	//	if (mcl_itime_leq_0(mclcb->tx_mem_cleanup_rem_it)) {
	//		if (mclcb->first_tx_for_mclcb == 0) {
	//			/* clean only after tx start */
	//			mcl_tx_cleanup(mclcb);
	//		}
	//		mclcb->tx_mem_cleanup_rem_it = mcl_usec2it(TX_MEM_CLEANUP_PERIOD);
	//	}
