/* $Id: mcl_fsm.cpp,v 1.3 2004/01/30 16:27:42 roca Exp $ */
/*
 ...
 *  |            | | |     ^                                       |
 *  |            | | |     |  there are remaining DUs              |
 *  +------------+ | +-----+  or pending ctrl operations           |
 *  |              |                                               |
 *  |              | all DUs have been tx & no pending ctrl        |
 *  |              |                                               |
 *  +------------+ | +---------------------------------------------+
 *               | | |                                                      
 *               v v v                                                      
...
/*
 * SENDER's FSM (Finite State Machine) transition table
 */
...
/*
 * RECEIVER's FSM (Finite State Machine) transition table
 */
...
static const mcl_rx_states next_rx_state[7/*state*/][9/*event*/] = {
   ...
   /* READY */ {xx,	xx,		RSTATE_IN_RX,	xx,		xx,		xx,		RSTATE_CLOSED,	RSTATE_CLOSE_RX,xx},
...
{
	mcl_tx_rx_mode	mode = mclcb->get_mode();
	if (mode == MCL_IS_A_SENDER_ONLY) {
		return (this->tx_state >= TSTATE_READY);
	} else if (mode == MCL_IS_A_RECEIVER_ONLY) {
